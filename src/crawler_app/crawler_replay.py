...
if __name__ == '__main__':
	...
	#
	if len(sys.argv) < 2:
		print 'Usage: crawl_replay.py <config file>\n'
		sys.stdout.flush()
		sys.exit(1)
	...
	try :
		...
		dictCrawlSpec = dictConfig['crawl_spec_replay']
		...
		# init source
		sourceObj = factoryObj.get_source( source_type = 'replay' )

		# start a crawl
		sourceObj.start(
			replay_dir = dictCrawlSpec['input_path'],
			file_pattern = dictCrawlSpec['file_pattern'],
			content_type = 'twitter',
			posts_per_hour = dictCrawlSpec['posts_per_hour'],
		...
		logger.exception('crawl_replay main() exception')

	finally :
		# stop the source (e.g. might be open if user aborted using cntrl-c) to avoid spawned python processes continuing after main process ends
		if sourceObj != None :
			sourceObj.stop()
			time.sleep(2)
